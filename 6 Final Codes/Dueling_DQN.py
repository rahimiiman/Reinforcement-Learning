...
HIDDEN_SIZE=128
value_net_cfg=[env.observation_space.shape[0],HIDDEN_SIZE,'ReLU',HIDDEN_SIZE,1]
adv_net_cfg=[env.observation_space.shape[0],HIDDEN_SIZE,'ReLU',HIDDEN_SIZE,env.action_space.n]
Dueling_net=models.DuelingDQNLinear(value_net_cfg,adv_net_cfg)
agent=models.DQNAgent(Dueling_net)           #agent functionality is exactly same as DQN
tgt_net=models.TargetNet(Dueling_net)
...
optimizer=optim.Adam(Dueling_net.parameters(),HYPER_PARAMETERS['learning_rate'])
training_flag=True
best_reward=0
...
while training_flag:
    ...
    for idx,exp in enumerate(utilities.exp_source_first_last(env,agent,HYPER_PARAMETERS['gamma'],HYPER_PARAMETERS['n_step_unrolling'],epsilon_obj)):
        ...
        output_metrics['Epsilon'].append(agent.epsilon)
        if len(replay_buffer_obj) < HYPER_PARAMETERS['REPLAY_BUFFER_SIZE'] /5:
            continue
        ...
        optimizer.zero_grad()
        q_prime_s_a_bellman=models.bellman_equation_estimator_Double_DQN(Dueling_net,tgt_net,reward_batch_tensor,HYPER_PARAMETERS['gamma'],last_state_batch_tensor,done_trunc_tensor,HYPER_PARAMETERS['n_step_unrolling'])
        
        q_s_a_matrix = Dueling_net(state_batch_tensor)
        ...
        if len(moving_average_reward)>0:
            if moving_average_reward[-1] > best_reward:
                ...
                torch.save(Dueling_net.state_dict(),'Dueling_DQN.pth')
...
Dueling_net.load_state_dict(torch.load("Dueling_DQN.pth"))  # Load the best model
Dueling_net.eval()  # Set to evaluation mode
agent=models.DQNAgent(Dueling_net)
